coreApp.controller("WelcomeCtrl", function ($scope, GlobalSvc, OptionSvc, DaoSvc, UserCodeSvc,$modal, $log, SyncSvc, $window, $location, $http,Settings, $route) {
    //DaoSvc.openDB();
    //OptionSvc.init();
    ...
    /*
     ...
     var url = GlobalSvc.resturl() + 'activities2/' + $scope.id;
     console.log(url);
     GlobalSvc.postData(url,
     $scope.Register,
     function(){
     $scope.message = 'Thank you for registering, we will contact you short';
     $scope.$apply();
     },
     function(){alert('Error Creating Issue');},
     'Activities',
     'Modify'
     );
     };
     */
    //$scope.myModal = $modal({scope: $scope, template: 'js/core/modal/login.html', show : 'false'});
    /*
     *Code From the modal instance controller 
     */
        ...
        $http({method: 'GET', url: url})
            .success(function(json, status, headers, config) {
                ...
                if (status) {
                    ...
                    //$scope.loggedIn = true;
                    localStorage.setItem('loggedIn','true');
                    DaoSvc.openDB(function(){SyncSvc.sync(GlobalSvc.getUser().SupplierID, GlobalSvc.getUser().UserID, $scope)});
                    //SyncSvc.sync(GlobalSvc.getUser().SupplierID, GlobalSvc.getUser().UserID, $scope);
                    //$location.path('/main');
                    //loginjs.getInstance().sync();
                } else {
    ...
    function constructor(){
        //first time sign in
        if (GlobalSvc.getUser().UserID.length===0) {
