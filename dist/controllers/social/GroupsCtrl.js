coreApp.controller("GroupsCtrl", function ($scope, GlobalSvc, JsonFormSvc, Settings, $routeParams, $http,$modal,$route,$alert,$location,$window) {
    ...
    $scope.onfetch = function(json){
        ...
        for (var x=0; x< json.length; x++){
            var row = json[x];
            if (row.image) {
                row.image = Settings.imageUrl + '?id=' + row.image + '&supplierID=' + GlobalSvc.getUser().SupplierID + '&width=' + size + '&height=' + size;
            } else {
    ...
    $scope.saveEvent = function(){
        ...
        if($scope.event.Deleted === true){
            ...
            if($scope.event.Duration === 0){ $scope.errMsg = "Please enter the event's duration  Duration such as 1.23 for 1 hour 23 minutes"; return;};
            ...
            var DueDate = new Date($scope.event.duedate);
            var DueTime = new Date($scope.event.duetime);
            $scope.event.duedate = DueDate.getFullYear()+'-'+ (DueDate.getMonth() + 1) +'-'+DueDate.getDate();
            $scope.event.duetime = DueTime.getHours()+':'+ DueTime.getMinutes() +':'+ DueTime.getSeconds();
